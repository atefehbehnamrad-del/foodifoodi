...
public:
    ...
    double calculateTotal() {
        double t = 0;
        for (int i = 0; i < items.size(); i++) t += items[i].getTotal();
        return t;
    }
...
public:
    ...
    void registerUser() {
        ...
        cout << "You Registered successfully! Your ID:] " << id << endl;
    }
    ...
    bool login() {
        ...
        for (int i = 0; i < users.size(); i++) {
            if (users[i].getUsername() == username && users[i].login(password)) {
                currentUser = &users[i];
                cout << "Welcome " << username << "! Hope you have a good time here:]" << endl;
                return true;
            }
        }
        cout << "Error: The password or the username you enter is incorroct.Try again." << endl;
        return false;
    }
...
public:
    ...
    if (restaurants.empty()) { 
        cout << "No restaurants found yet." << endl; 
        return; 
    }
...
public:
    ...
    void showRestaurantReviews(int restaurantId) {
        ...
        for (int i = 0; i < reviews.size(); i++) {
            if (reviews[i].getRestaurantId() == restaurantId) {
            reviews[i].display(); cout << "---\n"; found = true;
            }
        }

        
        if (!found){
            cout << "No reviews found yet." << endl;
        } 
    }
};
...
void userMenu(User* user, RestaurantManager& rm, Cart& cart, vector<Order>& orders, ReviewManager& reviewManager) {
    ...
    do {
        cout << "\n***** USER MENU *****" << endl;
        cout << "1. Finding Restaurants" << endl;
...
void mainMenu() {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            cout << "Thanks for choosing us:) Goodbye!" << endl;
        } else {
            cout << "Error: choose a correct number." << endl;
        }
    } while (choice != 0);
