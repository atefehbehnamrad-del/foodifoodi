...
private:
    MenuItem* item;
    double price;   // FIX: was uppercase Price
    int count;      // FIX: was uppercase Count

public:
    OrderItem(MenuItem* i, int c)
        : item(i), count(c), price(i->getPrice()) {}  // FIX: getPrice() not getPrice

    double getTotal() const { return count * price; }
    MenuItem* getItem() const { return item; }
    int getCount() const { return count; }
    double getPrice() const { return price; }  // FIX: added missing semicolon

    void Display() const {
        cout << "Item: " << item->getName() << endl;  // FIX: was missing semicolon
...
private:
    vector<User> users;
    User* currentUser;  // FIX: was PeresentUser
...
public:
    ...
    void registerUser() {
        ...
        cout << "Enter username: "; cin >> username;   // FIX: removed >> endl
    ...
    bool login() {
        ...
        cout << "Username: ";
        cin >> username;         // FIX: removed >> endl
        cout << "Password: ";
        cin >> password;
        for (int i = 0; i < users.size(); i++) {
            if (users[i].getUsername() == username && users[i].login(password)) {
                currentUser = &users[i];              // FIX: was PeresentUser
    ...
    User* getCurrentUser() { return currentUser; }   // FIX: was getPeresentUser
...
public:
    void addRestaurant() {
        ...
        cin >> name;      // FIX: removed >> endl
        cout << "Address: "; 
        cin >> address;
        cout << "Phone: "; 
        cin >> phone;               // FIX: was cin << phone
        cout << "Status (1=open, 0=closed): "; 
        cin >> status;
        cout << "Prepare time: "; 
        cin >> prepareTime;
        cout << "Description: "; 
    ...
    Restaurant* findRestaurant(int id) {
        for (auto& r : restaurants) {           // FIX: was missing closing brace for for-loop
...
public:
    void addReview(int userId, int restaurantId) {
        ...
        cin >> rating;
        cout << "Your comment: "; 
        cin.ignore(); getline(cin, comment);
    ...
    void showRestaurantReviews(int restaurantId) {
        ...
        bool found = false;
        for (auto& r : reviews) {
            if (r.getRestaurantId() == restaurantId) { r.display(); cout << "---" << endl; found = true; }
        }
...
void userMenu(User* user, RestaurantManager& rm, Cart& cart, vector<Order>& orders, ReviewManager& reviewManager) {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            Restaurant* r = rm.findRestaurant(restaurantId);  // FIX: was restId
            if (r) r->displayMenu();
            else cout << "Restaurant not found!" << endl;
        } else if (choice == 3) {
            int restaurantId, itemId, count;         // FIX: removed stray Persian char
            cout << "Restaurant ID: "; cin >> restaurantId;  // FIX: removed >> endl
            ...
            if (cart.getTotal() > 0) {
                ...
                cout << "How will you pay? (cash/card): ";
    ...
    } while (choice != 0);  // FIX: was missing closing brace for do-while
}
...
void adminMenu(RestaurantManager& rm) {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            int newId = 100 + (int)rm.findRestaurant(restaurantId)->getId() * 10;
            // Use a simple unique ID based on time to avoid collisions
...
void mainMenu() {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            restaurantManager.displayAllRestaurants();  // FIX: was rm.displayAllRestaurants()
        } else if (choice == 4) {
            cart.display();
        } else if (choice == 5) {
            for (auto& order : orders) order.display();
