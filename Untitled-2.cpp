...
class Cart {
private:
    // item pointer, count, restaurantId
    vector<tuple<MenuItem*, int, int>> items;
...
public:
    void addItem(MenuItem* item, int count, int restaurantId) {
        ...
        for (auto& entry : items) {
            if (get<0>(entry)->getId() == item->getId()) {
                get<1>(entry) += count;
                cout << item->getName() << ": " << count << " more added." << endl;
                return;
            }
        }
        items.push_back(make_tuple(item, count, restaurantId));
        cout << item->getName() << " added. Count: " << count << endl;
    }

    void removeItem(int itemId) {
        for (int i = 0; i < (int)items.size(); i++) {
            if (get<0>(items[i])->getId() == itemId) {
                items.erase(items.begin() + i);
    ...
    void update(int itemId, int newCount) {
        ...
        for (auto& entry : items) {
            if (get<0>(entry)->getId() == itemId) {
                get<1>(entry) = newCount;
    ...
    double getTotal() const {
        ...
        for (const auto& entry : items)
            total += get<0>(entry)->getPrice() * get<1>(entry);
        return total;
    }
    ...
    void display() const {
        ...
        cout << "***** Cart *****" << endl;
        for (const auto& entry : items) {
            cout << "   " << get<1>(entry) << " * " << get<0>(entry)->getName()
                 << " = " << get<0>(entry)->getPrice() * get<1>(entry)
                 << "$ (Restaurant ID: " << get<2>(entry) << ")" << endl;
        }
        cout << "Total: " << getTotal() << "$" << endl;
    }

    // Shows previous orders, then creates one Order per restaurant from cart items
    void checkout(int userId, vector<Order>& orders) {
        if (items.empty()) {
            cout << "The cart is empty!" << endl;
            return;
        }

        // Show previous orders before confirming
        if (!orders.empty()) {
            cout << "\n--- Your Previous Orders ---" << endl;
            for (auto& o : orders) o.display();
            cout << "----------------------------" << endl;
        }

        // Group items by restaurant ID
        map<int, vector<pair<MenuItem*, int>>> grouped;
        for (auto& entry : items)
            grouped[get<2>(entry)].push_back({get<0>(entry), get<1>(entry)});

        // Create one order per restaurant
        for (auto& kv : grouped) {
            int rid = kv.first;
            Order newOrder((int)orders.size() + 1, userId, rid);
            for (auto& itemPair : kv.second)
                newOrder.addItem(itemPair.first, itemPair.second);
            cout << "Order #" << newOrder.getId()
                 << " placed for Restaurant ID " << rid
                 << " — Total: " << newOrder.calculateTotal() << "$" << endl;
    ...
    do {
        ...
        if (choice == 1) {
            ...
            if (r) {
                ...
                if (item) cart.addItem(item, count, restaurantId); // pass restaurantId
            ...
            if (cart.getTotal() > 0) {
                cart.checkout(user->getId(), orders); // no restaurantId needed
                if (!orders.empty()) {
                    string method;
            ...
            if (orders.empty()) cout << "No orders yet." << endl;
